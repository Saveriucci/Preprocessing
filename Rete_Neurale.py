...
# Matrice di confusione e precision/recall
labels, preds = evaluate_model(model, test_loader)
cm = confusion_matrix(labels, preds)
sns.heatmap(cm, annot=True, fmt="d", cmap="Blues", xticklabels=df['organ'].unique(), yticklabels=df['organ'].unique())
...
# UMAP & t-SNE comparativo
umap_model = umap.UMAP()
tsne_model = TSNE(n_components=2)

umap_embeddings = umap_model.fit_transform(preds)
tsne_embeddings = tsne_model.fit_transform(preds)
